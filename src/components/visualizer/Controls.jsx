...
import {
  ...
  RotateCcw, Shuffle, ChevronRight, ChevronLeft,
} from 'lucide-react'
          ...
          <button
            ...
            onMouseEnter={e => {
              if (!disabled) {
                ...
                if (variant === 'play') e.currentTarget.style.boxShadow = '0 0 24px #AAFF0066'
                else e.currentTarget.style.borderColor = '#AAFF0055'
              }
            }}
            onMouseLeave={e => {
              e.currentTarget.style.transform = 'translateY(0)'
              if (variant === 'play') e.currentTarget.style.boxShadow = '0 0 16px #AAFF0044'
...
// ─── Radix Slider (21st.dev-style) ───────────────────────────
function StyledSlider({ value, onValueChange, min, max, step, label, displayValue }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minWidth: '120px' }}>
      ...
      <Slider.Root
        ...
        style={{
          ...
          touchAction: 'none', width: '100%', height: '20px',
        }}
      >
        <Slider.Track style={{
          background: 'var(--border)', position: 'relative',
          flexGrow: 1, borderRadius: '9999px', height: '4px',
        }}>
          <Slider.Range style={{
            position: 'absolute', background: '#AAFF00',
            borderRadius: '9999px', height: '100%',
          }} />
        </Slider.Track>
        <Slider.Thumb style={{
          display: 'block', width: '16px', height: '16px',
          background: '#AAFF00', borderRadius: '9999px',
          boxShadow: '0 0 8px #AAFF0077',
          cursor: 'pointer', outline: 'none',
          transition: 'transform 0.1s, box-shadow 0.1s',
        }}
          onMouseEnter={e => {
            e.currentTarget.style.transform = 'scale(1.2)'
            e.currentTarget.style.boxShadow = '0 0 14px #AAFF00AA'
          }}
          onMouseLeave={e => {
            e.currentTarget.style.transform = 'scale(1)'
            e.currentTarget.style.boxShadow = '0 0 8px #AAFF0077'
          }}
        />
      </Slider.Root>
    </div>
    ...
    <div style={{
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          <span style={{ fontSize: '9px', letterSpacing: '0.15em', color: 'var(--text-muted)', fontFamily: 'Space Mono, monospace' }}>
            PROGRESS
          </span>
        ...
        <div style={{
          ...
          background: 'var(--border)', borderRadius: '9999px', overflow: 'hidden',
        }}>
          <div style={{
            width: `${progress}%`, height: '100%',
            background: isFinished ? 'var(--bar-sorted)' : '#AAFF00',
            borderRadius: '9999px',
            transition: 'width 0.2s ease',
            boxShadow: isFinished ? 'none' : '0 0 8px #AAFF0066',
          }} />
        </div>
      </div>
      ...
      <div style={{
        ...
        <TipButton tip="Step back" onClick={onStepBack} disabled={currentStep === 0}>
          <ChevronLeft size={15} />
        </TipButton>

        <TipButton tip="Reset" onClick={onReset}>
          <RotateCcw size={14} />
        </TipButton>

        <TipButton
          tip={isPlaying ? 'Pause' : isFinished ? 'Replay' : 'Play'}
        ...
        <TipButton tip="Randomize array" onClick={onRandomize}>
          <Shuffle size={14} />
        </TipButton>

        <TipButton tip="Step forward" onClick={onStepForward} disabled={isFinished}>
