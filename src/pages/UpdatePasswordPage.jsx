...
export default function UpdatePasswordPage() {
  ...
  useEffect(() => {
    // PRIMARY METHOD: Listen for PASSWORD_RECOVERY event from Supabase
    // This fires automatically when user clicks the reset link
    ...
    // FALLBACK METHOD: Check if tokens are in URL hash (older Supabase versions)
    const checkHash = async () => {
      ...
      if (hash && (hash.includes('type=recovery') || hash.includes('access_token'))) {
        ...
        // No hash — check if there's already an active recovery session
        const { data } = await supabase.auth.getSession()
        if (data.session) {
          setSessionReady(true)
        } else {
          // Wait for the onAuthStateChange event
          // Set a timeout — if nothing happens in 8 seconds, show error
          setTimeout(async () => {
  ...
  const isDisabled = loading || !passwordIsStrong(password) || password !== confirm || confirm === ''
    ...
    <div style={{ minHeight:'100vh', background:'var(--bg-base)', display:'flex', alignItems:'center', justifyContent:'center', padding:'24px', fontFamily:'DM Sans,sans-serif' }}>
      <div style={{ width:'100%', maxWidth:'420px' }}>
        ...
        <div style={{ background:'var(--bg-surface)', border:'1px solid var(--border)', borderRadius:'14px', padding:'32px' }}>
          ...
          {/* Success */}
          {isSuccess && (
            <div style={{ padding:'16px', borderRadius:'10px', background:'#AAFF0011', border:'1px solid #AAFF0033', color:'#AAFF00', fontFamily:'Space Mono,monospace', fontSize:'13px', textAlign:'center' }}>
              ✓ {message}
            </div>
          )}
            ...
            <form onSubmit={handleUpdate} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  style={{ width:'100%', padding:'12px 16px', background:'var(--bg-elevated)', border:`1px solid ${confirm && confirm !== password ? '#FF4444' : confirm && confirm === password ? '#AAFF00' : 'var(--border)'}`, borderRadius:'8px', color:'var(--text-primary)', fontSize:'14px', fontFamily:'DM Sans,sans-serif', outline:'none', boxSizing:'border-box', transition:'border-color 0.2s' }}
                  onFocus={e => e.target.style.borderColor = '#AAFF00'}
                  onBlur={e => e.target.style.borderColor = confirm && confirm !== password ? '#FF4444' : confirm && confirm === password ? '#AAFF00' : 'var(--border)'}
                />
                {confirm && confirm !== password && (
                  <p style={{ fontSize:'11px', color:'#FF4444', marginTop:'4px' }}>Passwords do not match</p>
                )}
                {confirm && confirm === password && passwordIsStrong(password) && (
                  <p style={{ fontSize:'11px', color:'#AAFF00', marginTop:'4px' }}>✓ Passwords match</p>
                )}
              </div>
              ...
              <button
                ...
                style={{
                  padding:'14px',
                  background: isDisabled ? '#AAFF0033' : '#AAFF00',
                  color: isDisabled ? '#0D0D0D66' : '#0D0D0D',
                  border:'none', borderRadius:'8px',
                  fontFamily:'Space Mono,monospace', fontSize:'13px', fontWeight:700,
                  letterSpacing:'0.12em',
                  cursor: isDisabled ? 'not-allowed' : 'pointer',
                  transition:'all 0.2s',
                  boxShadow: isDisabled ? 'none' : '0 0 20px #AAFF0033',
                }}
              >
          ...
          {!sessionReady && !sessionError && !isSuccess && (
            <div style={{ textAlign:'center', padding:'32px 20px' }}>
              <div style={{ fontFamily:'Space Mono,monospace', fontSize:'12px', color:'var(--text-muted)', marginBottom:'12px' }}>
